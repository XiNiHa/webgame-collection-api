...
pub fn node_ident_impl(input: TokenStream) -> TokenStream {
...
fn map_variants(variant: &Variant) -> MappedVariant {
    ...
    let meta_list = variant
        .attrs
        .iter()
        .find(|attr| attr.path.is_ident("node_ident"))
        .and_then(|attr| match attr.parse_meta() {
            Ok(Meta::List(list)) => Some(list.nested),
            _ => None,
        });
    let str_lit = LitStr::new(&ident.to_string(), ident.span());
    let mut resolver_fn_ident = None;

    if let Some(meta_list) = meta_list {
        for meta in meta_list {
            if let NestedMeta::Meta(Meta::NameValue(mnv)) = meta {
